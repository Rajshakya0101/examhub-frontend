...
/**
 * Verify Firebase configuration and connection
 * This will check if your Firebase configuration is valid and working
 */
export const verifyFirebaseConfig = async () => {
  console.log('Verifying Firebase configuration...');
  
  try {
    // Make sure we're in a browser environment with import.meta available
    if (typeof window === 'undefined' || typeof import.meta === 'undefined') {
      throw new Error('This function is designed to run in a browser environment with Vite');
    }
    
    // First, extract environment variables directly
